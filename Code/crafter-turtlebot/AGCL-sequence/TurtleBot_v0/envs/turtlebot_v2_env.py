...
class TurtleBotV2Env(gym.Env):
	...
	def get_observation(self):
		...
		while True:
			...
			beam_i = np.zeros(num_obj_types) # this size is equal to the number of objects present
				...
				for i in range(self.n_trees+self.n_rocks+self.n_table):
					if x > self.x_low[i] and x < self.x_high[i]:
						if y > self.y_low[i] and y < self.y_high[i]:
							...
							index_temp+=1
							beam_i[obj_type] = sensor_value #I find out the distance of the object, what object it is. and I update 
