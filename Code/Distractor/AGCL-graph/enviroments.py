...
def set_parameters_for_model():
    global no_of_environmets, width_array, height_array, no_trees_array, no_rocks_array, table_array, no_fires_array,\
        tents_area, starting_trees_array, starting_rocks_array, starting_pogo_sticks_array, type_of_env_array, env_id, random_objects
...
def get_envs():
    ...
    envs = []
    for i_env, (width, height, no_tree, no_rock, no_table, start_tree, start_rock, type_of_env, no_random) in \
            enumerate(zip(width_array, height_array, no_trees_array, no_rocks_array, table_array, starting_trees_array, starting_rocks_array, type_of_env_array, random_objects)):
        env = gym.make(env_id,
                       map_width=width,
                       map_height=height,
                       items_quantity={'tree': no_tree, 'rock': no_rock, 'crafting_table': no_table, 'random_obj': no_random,
                                       'pogo_stick': 0},
                       initial_inventory={'wall': 0, 'tree': start_tree, 'rock': start_rock,
                                          'crafting_table': 0, 'pogo_stick': 0, 'random_obj':0,
                                          },
